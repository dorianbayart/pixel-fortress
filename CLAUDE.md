...
- **IMPORTANT — Deploy workflow & stable vs. unstable:** The `deploy.yml` CI workflow deploys to two locations on the `gh-pages` branch:
  - **`/unstable/`** — always updated on every push to `main` (reflects the current state of the repo regardless of version).
  - **`/` (root, stable)** — only updated when `package.json` version has **no pre-release suffix** (e.g. `0.0.4`, not `0.0.5-unstable`). Game files (`play.html`, `js/`, `assets/`, `css/game-*.css`, etc.) at root are intentionally frozen at the last stable release.
  - **Landing/static pages** (`index.html`, translated pages, `release-notes.html`, `sitemap.xml`, `css/landing.css`, `css/release-notes.css`) are deployed to root on **every** push, so they always stay current even between stable releases.
  - **Consequence:** changes to landing page templates, locale files, or release notes are immediately visible on the live site after any push. Changes to game code only reach the stable root when a version without a `-` suffix is released.
- **IMPORTANT — Sitemap:** Never edit `sitemap.xml` directly. It is auto-generated by `scripts/build-sitemap.js` from the list of pages defined in that script. Rebuild with `node scripts/build-sitemap.js` (or `npm run build-sitemap`). `lastmod` dates are derived from `git log` on each page's source files, so a full git history (`fetch-depth: 0`) is required in CI. Add new public pages to the `PAGES` array in the script.
